...
interface SignupFormProps {
  onSignup: (email: string, password: string, fullName: string, username: string) => void;
...
export default function SignupForm({ onSignup, onSwitchToLogin }: SignupFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSignup(email, password, fullName, username);
  };
    ...
    <div className="w-full max-w-md mx-auto">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 border border-gray-100 dark:border-gray-700">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2 font-serif">Connect with fellow book lovers</p>
        </div>
